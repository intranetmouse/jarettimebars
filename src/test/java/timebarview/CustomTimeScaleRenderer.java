 ...
 *
 * @author Peter Kliem
 * @version $Id: DefaultTimeScaleRenderer.java 856 2009-04-02 18:54:40Z kliem $
 */
public class CustomTimeScaleRenderer implements TimeScaleRenderer, ITickProvider {
    ...
    /** rendering component. */
    protected MyTimeScaleRenderer _renderer;
    private Format _format;

    public CustomTimeScaleRenderer(Format format) {
        _format = format;
        _renderer = new MyTimeScaleRenderer();
    }
    ...
    protected void initIterators() {
        ...
        DateIterator iterator = new SecondIterator(1);
        ...
        iterator = new timebarview.MinuteIterator(1);
        _iterators.add(iterator);
        _formats.add(_format);

        iterator = new timebarview.MinuteIterator(10);
        _iterators.add(iterator);
        _formats.add(_format);
            ...
            if (horizontal) {
                ...
                if (_midStrip != null) {
                    ...
                    while (it.hasNextDate()) {
                        ...
                        String label = it.getLabel(d, _format);
        ...
        private void checkStrips(Graphics gc, TimeBarViewerDelegate delegate, JaretDate startDate) {
            for (int i = 0; i < _iterators.size(); i++) {
                ...
                if (it.previewNextDate() != null) {
                    ...
                    String label = it.getLabel(current, _format);
                    Rectangle2D rect = gc.getFontMetrics().getStringBounds(label, gc);
                    int bonus = (_midStrip == it && _formats.get(i).equals(Format.LONG)) ? SETBONUS : 0;
